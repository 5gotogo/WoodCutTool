...
const responsiveSrc = (src) => src.endsWith(".webp") ? src.replace(/\.webp$/, "-800.webp") : src;
const visualFigure = (src, alt, { wide = false, eager = false } = {}) => `<figure class="visual-frame${wide ? " wide" : ""}"><img src="${src}" srcset="${responsiveSrc(src)} 800w, ${src} ${wide ? 1600 : 1200}w" sizes="(max-width: 820px) calc(100vw - 32px), ${wide ? "760px" : "520px"}" alt="${alt}" width="${wide ? 1600 : 1200}" height="900" ${eager ? 'loading="eager" fetchpriority="high"' : 'loading="lazy"'} decoding="async"></figure>`;
...
<head>
  ...
  <link rel="preload" as="image" href="/assets/images/tools/tools-hero-800.webp" imagesrcset="/assets/images/tools/tools-hero-800.webp 800w, /assets/images/tools/tools-hero.webp 1600w" imagesizes="(max-width: 820px) calc(100vw - 32px), 760px" fetchpriority="high"><style>.mega-menu{display:none}</style><link rel="stylesheet" href="/assets/styles.css"><script defer src="/assets/site-chrome.js"></script><script defer src="/assets/app.js"></script>
...
<body>
  ...
  <main id="main">
    ...
    <section class="section"><div class="grid tools">${card("Material & Cost Planning", "Estimate six practical panel jobs, review purchase allowances, then plan the actual cuts in CutList.", "/estimating/", "Start an estimate")}${card("Woodworking Tools", "Cut and layout, cabinet and furniture, wood species, sheet goods, board feet, material cost, and shop measurement tools.", "/tools/woodworking/", "Browse woodworking tools")}${card("Construction Tools", "Stairs, tile, deck, fence, wall, roof geometry, concrete, spacing, finish, and field-planning calculators.", "/tools/construction/", "Browse construction tools")}</div></section>
...
const woodworkingGroups = [
  ...
  { eyebrow: "Wood & Materials", title: "Wood and material tools", description: "Estimate lumber volume, weight, sheet goods, costs, and material properties.", cards: [
    ...
    { name: "Material & Cost Planning", description: "Estimate project panel allowances and costs, then continue with a checked CutList plan.", href: "/estimating/" },
